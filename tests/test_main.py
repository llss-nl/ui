...
def test_firewall_group():
    with requests_mock.Mocker() as m:
        m.get('https://192.168.100.1/proxy/network/api/s/default/rest/firewallgroup/662fa7f339ff5e79202dd1bd', json={"data": []})
        api = UnifyAPI()
...
def test_add_alarms():
    with requests_mock.Mocker() as m:
        m.get('https://192.168.100.1/proxy/network/api/s/default/stat/alarm', json={"data": [{"src_ip": "192.168.1.1"}]})
        api = UnifyAPI()
        ips = add_alarms(api, [])
        assert ips == []
